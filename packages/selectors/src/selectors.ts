...
export type SelectorBase<TState, TReturn> = (state: TState, ...args: any[]) => TReturn

export interface SelectorsBase<TState> { [name: string]: SelectorBase<TState, any> }

export interface ResolvedSelectorExtras<TArgs extends any[], TReturn> {
  bound: (...args: TArgs) => TReturn
}

export type ResolvedSelector<TState, TSelector extends SelectorBase<TState, ReturnType<TSelector>>> =
  TSelector extends (state: TState, ...args: infer TArgs) => infer TReturn
  ? ((state: TState, ...args: TArgs) => TReturn) & ResolvedSelectorExtras<TArgs, TReturn>
  : never

export type ResolvedSelectors<TState, TSelectors extends SelectorsBase<TState>> = {
  [name in keyof TSelectors]: ResolvedSelector<TState, TSelectors[name]>
}

export type SelectorsFactory<TState> = <TSelectors extends SelectorsBase<TState>>(selectors: TSelectors) => ResolvedSelectors<TState, TSelectors>
...
declare module '@simplux/core' {
  export interface SimpluxModule<TState> extends SimpluxModuleSelectorExtensions<TState> { }
}

export const selectorsModuleExtension: SimpluxModuleExtension<SimpluxModuleSelectorExtensions<any>> = (
  { name },
  { getState },
  moduleExtensionStateContainer,
) => {
  moduleExtensionStateContainer.selectors = moduleExtensionStateContainer.selectors || {}
  const moduleSelectors: SelectorsBase<unknown> = moduleExtensionStateContainer.selectors[name] = moduleExtensionStateContainer.selectors[name] || {}
  return {
